...
function BrowserFrame({ children }: { children: ReactNode }) {
  return (
    <div className="rounded-[2rem] border border-mist bg-paper p-3 shadow-2xl shadow-mist/60">
      <div className="overflow-hidden rounded-[1.5rem] border border-mist bg-white">
        ...
        <div className="relative aspect-[16/10] overflow-hidden bg-gradient-to-br from-[#f6f6f6] via-mist to-[#c9d1d6]">
          {children}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="grid h-full grid-cols-3 gap-4 p-8">
      {Array.from({ length: 9 }).map((_, index) => (
        ...
        >
          <div className="m-3 h-2 rounded-full bg-steel/25" />
          <div className="mx-3 h-14 rounded-xl bg-mist/70" />
        </div>
      ))}
    </div>
  );
...
function BackgroundRemovalMockup() {
  return (
    <>
      ...
      <div className="absolute right-8 top-8 rounded-full bg-paper/85 px-4 py-2 text-xs font-semibold text-ink shadow-sm ring-1 ring-mist">
        Background removed
      </div>
    </>
  );
