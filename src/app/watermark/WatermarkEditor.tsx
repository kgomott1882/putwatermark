...
import {
  ...
  revokeLogoLayerUrls,
  SINGLE_LOGO_WATERMARK_DEFAULTS,
...
export default function WatermarkEditor() {
  ...
  function handleTextWatermarkChange(value: string) {
    const isFirstText =
      Boolean(value.trim()) && !activeTextLayer.text.trim();
    ...
    if (isFirstText) {
      ...
      patch.watermarkPosition = defaults.watermarkPosition;
      patch.fontSizeScale = defaults.fontSizeScale;
      patch.textColor = defaults.textColor;
  ...
  function handleMobileWatermarkTextFocus() {
    if (activeTextLayer.text.trim()) {
      return;
    }
    ...
    if (
      activeTextLayer.customPosition ||
      activeTextLayer.watermarkPosition !== defaults.watermarkPosition ||
      activeTextLayer.fontSizeScale !== defaults.fontSizeScale
    ) {
  ...
  function applyLogoWatermarkModeDefaults(mode: WatermarkMode) {
    ...
    const defaults = SINGLE_LOGO_WATERMARK_DEFAULTS;
  ...
  function handleWatermarkToolSelect(tool: WatermarkToolId) {
    ...
    if (tool === "text") {
      const shouldApplySingleDefaults =
        ...
        (!activeTextLayer.text.trim() && watermarkMode === "tile");
    ...
    if (tool === "logo") {
      const shouldApplySingleDefaults =
        ...
        (!activeLogoLayer.logoImage && watermarkMode === "tile");

      handleWatermarkTypeChange("logo");

      if (shouldApplySingleDefaults) {
        applyLogoWatermarkModeDefaults("single");
      }
  ...
  function loadLogoFile(file: File) {
    ...
    nextLogo.onload = () => {
      const singleDefaults = SINGLE_LOGO_WATERMARK_DEFAULTS;
      const tileTemplate =
        watermarkMode === "tile"
          ? logoWatermarkTemplates.find(
              (entry) => entry.id === "logo-protect-light",
            )
          : null;
      const layerPatch: Partial<LogoWatermarkLayer> = {
        backgroundRemovedLogoImage: null,
        customPosition: null,
      ...
      if (watermarkMode === "single") {
        layerPatch.customPosition = singleDefaults.customPosition;
        layerPatch.fontSizeScale = singleDefaults.fontSizeScale;
        layerPatch.opacity = singleDefaults.opacity;
        layerPatch.watermarkPosition = singleDefaults.watermarkPosition;
      } else if (tileTemplate) {
        layerPatch.customPosition = null;
        layerPatch.fontSizeScale = tileTemplate.fontSizeScale;
        layerPatch.opacity = tileTemplate.opacity;
        layerPatch.watermarkPosition = tileTemplate.position;
      }

      updateLogoLayer(activeLogoLayerId, layerPatch);
      setWatermarkType("logo");
      setActiveWatermarkTool("logo");
      ...
      if (watermarkMode === "single") {
        setFontSizeScale(singleDefaults.fontSizeScale);
        setWatermarkOpacity(singleDefaults.opacity);
        setWatermarkPosition(singleDefaults.watermarkPosition);
        clearActiveLogoTemplate();
        return;
      }

      if (tileTemplate) {
        setWatermarkMode("tile");
        setTileAngle(tileTemplate.tileAngle);
        setTileDensity(tileTemplate.density);
        setTileGap(tileTemplate.tileGap);
        setFontSizeScale(tileTemplate.fontSizeScale);
        setWatermarkOpacity(tileTemplate.opacity);
        setWatermarkPosition(tileTemplate.position);
        setActiveLogoTemplate(tileTemplate.id);
      }
    };
    nextLogo.onerror = () => {
      URL.revokeObjectURL(objectUrl);
              ...
              {activeWatermarkTool === "text" ? (
                ...
                <div className="md:hidden">
                  <WatermarkMobileBar
                    ...
                    onTextChange={handleTextWatermarkChange}
                    onTextFocus={handleMobileWatermarkTextFocus}
                    onTextColorChange={(value) => {
                      if (shouldIgnoreManualSettingsChange()) {
                        return;
                      }
...
function getWatermarkCoordinates({
  ...
  const verticalPosition: "top" | "center" | "bottom" =
    position === "center"
      ? "center"
      : (position.split("-")[0] as "top" | "center" | "bottom");
  const horizontalPosition: "left" | "center" | "right" =
    position === "center"
      ? "center"
      : (position.split("-")[1] as "left" | "center" | "right");
