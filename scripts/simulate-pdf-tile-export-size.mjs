...
async function buildReusedTilePdf(originalBytes, unitPngBytes, centers, tileWidth, tileHeight) {
  const pdfDoc = await PDFDocument.load(originalBytes);
  const tileImage = await pdfDoc.embedPng(unitPngBytes);
...
function renderLegacyFullPageOverlay(unitCanvas, tileWidth, tileHeight) {
  const densitySpacing = pageW / repetitionsAcross;
  const diagonal = Math.hypot(pageW, pageH);
  const gapPixels = Math.max(tileHeight, tileWidth * (gapPercent / 100));
  const xSpacing = Math.max(densitySpacing, tileWidth + gapPixels);
  const ySpacing = Math.max(tileHeight * 2.4, densitySpacing * 0.65);
  const patternExtent = diagonal + Math.max(xSpacing, ySpacing) * 2;
  const canvas = createCanvas(pageW * exportScale, pageH * exportScale);
  const context = canvas.getContext("2d");
  context.scale(exportScale, exportScale);
  context.save();
  context.beginPath();
...
function renderTileUnitPng(unitCanvas, tileWidth, tileHeight) {
  return renderOrientedUnitPng(unitCanvas, tileWidth, tileHeight);
}
...
async function createOriginalPdf(pageCount = 2) {
  ...
  for (let index = 0; index < pageCount; index += 1) {
    ...
    page.drawText(
      "Vector text stays lightweight in the source document.",
      {
        opacity: 0.65,
        size: 11,
        x: 72,
...
async function buildLegacyPdf(originalBytes, overlayPngBytes) {
  const { PDFDocument: Doc } = await import("pdf-lib");
  const pdfDoc = await Doc.load(originalBytes);
  const embedded = await pdfDoc.embedPng(overlayPngBytes);
...
const legacyOverlayPng = renderLegacyFullPageOverlay(
  composite.canvas,
  tileWidth,
  tileHeight,
);
const orientedUnit = renderTileUnitPng(composite.canvas, tileWidth, tileHeight);
const originalBytes = await createOriginalPdf(2);
const legacyPdfBytes = await buildLegacyPdf(originalBytes, legacyOverlayPng);
const reusedTilePdfBytes = await buildReusedTilePdf(
  originalBytes,
  orientedUnit.png,
  centers,
...
writeFileSync(join(outputDir, "pdf-tile-unit.png"), orientedUnit.png);
writeFileSync(join(outputDir, "pdf-tile-legacy-overlay.png"), legacyOverlayPng);
writeFileSync(join(outputDir, "pdf-tile-reused-export.pdf"), reusedTilePdfBytes);
...
const result = {
  assumptions: {
    ...
    gapPercent,
    opacity,
    pagePoints: [pageW, pageH],
    pages: 2,
    tileDensity: "sparse",
  },
  currentApproach: {
    fullPageOverlayPixels: [pageW * exportScale, pageH * exportScale],
    overlayPngKB: Number((legacyOverlayPng.length / 1024).toFixed(1)),
    overlayPngPerTwoPagesKB: Number(((legacyOverlayPng.length * 2) / 1024).toFixed(1)),
    estimatedPdfKB: Number(((legacyOverlayPng.length * 2 + originalBytes.length) / 1024).toFixed(1)),
    simulatedPdfKB: Number((legacyPdfBytes.length / 1024).toFixed(1)),
  },
  newApproach: {
    embeddedTileImages: 1,
    tileDrawSizePixelsAt3x: [
      Math.round(tileWidth * exportScale),
      Math.round(tileHeight * exportScale),
    ],
    tileDrawSizePoints: [Number(tileWidth.toFixed(1)), Number(tileHeight.toFixed(1))],
    tilesPerPage: centers.length,
    orientedUnitBoundsPoints: [
      Number(orientedUnit.bounds.width.toFixed(1)),
      Number(orientedUnit.bounds.height.toFixed(1)),
    ],
    unitPngKB: Number((orientedUnit.png.length / 1024).toFixed(1)),
    simulatedPdfKB: Number((reusedTilePdfBytes.length / 1024).toFixed(1)),
  },
  userReportedCurrentPdfKB: 989,
};
